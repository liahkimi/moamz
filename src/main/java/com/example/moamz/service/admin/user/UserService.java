...
public class UserService {
    ...
    private final UserMapper userMapper; //의존성 주입 시, 한번만 초기화 가능

    //로그인 메서드 (회원번호 찾기)
    public Long findUserCode(String fgUserId, String fgUserPassword){
        return userMapper.selectUserCode(fgUserId, fgUserPassword) //userMapper의 메소드 호출
    ...
    public UserSessionDTO findLoginInfo(String fgUserId, String fgUserPassword){
        ...
        return userMapper.selectLoginInfo(fgUserId, fgUserPassword)
                .orElseThrow(() -> new IllegalStateException("존재하지 않는 회원 정보"));
